 	...
 	<tr>
 		...
 		<td>
 			<center>
 				...
 				<hr>
 				<em>
 					Jl. Ray Puspitek, Buaran,<br> Kac. Pamulang, Kota Tangerang Selatan, Banten 15310<br>
 				</em>
...
$cabut = mysqli_fetch_array(mysqli_query($con,"SELECT COUNT(ket) AS cabut FROM _logabsensi
	INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
		INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
		INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
 WHERE _logabsensi.id_mahasiswa='$ds[id_mahasiswa]' and _logabsensi.ket='C' and _logabsensi.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_mkelas='$_GET[kelas]'  AND tb_thajaran.status=1 AND tb_semester.status=1 "));
echo $cabut['cabut'];
